...
async function verifyAdminToken(req: express.Request): Promise<{ authenticated: boolean; isAdmin: boolean; error?: string; userId?: string }> {
  ...
  const handleUpload = (req: express.Request, res: express.Response) => {
    ...
    upload.single('file')(req, res, async (multerErr: any) => {
      ...
      try {
        ...
        console.log(`[Storage Upload Success] Bucket: ${bucket}, Path: ${filePath}, URL: ${publicUrl}`);
